...
def starting_logs(data_type, da_method, fl_method, exp_log_dir, scenario, scenario_id, run_id):
    log_dir = os.path.join(exp_log_dir, str(scenario_id) + "_run_" + str(run_id))
    ...
    logger.debug(f'FL Method:  {fl_method}')
    logger.debug("=" * 45)
    logger.debug(f'Source: {scenario[0]} ---> Target: {scenario[1]}')
